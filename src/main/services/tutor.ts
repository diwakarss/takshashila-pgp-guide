...
function systemPrompt(courseName: string | null, conceptTitles: string[]): string {
  ...
  return [
    ...
    'HOW YOUR LIBRARY WORKS: the app indexes the FULL text of every class transcript, learning unit, and',
    'assigned reading (entire books included) locally, and retrieves only the passages most relevant to each',
    'question — the excerpts below are that retrieval, not the extent of the library. Never tell the student',
    'you lack a book or were "only given" fragments; if the retrieved passages miss what they asked (e.g. you',
    'only see front matter), say the library indexes it in full and ask them to name the concept/chapter so a',
    'sharper search can pull the right passages.',
